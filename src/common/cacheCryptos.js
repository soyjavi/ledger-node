...
export default async (onlyLatest = false) => {
  ...
  const dates = Object.keys(await readFile("currencies.json"))
    .sort()
    .slice(0, -1);
    ...
    dates.forEach(async (key) => {
      tasks.push(
        new Promise(async (resolve) => {
          ...
          const { [BTC]: rates } = await getRates(
            `data/pricehistorical?fsym=${BTC}&ts=${date.getTime() / 1000}`
          );
          resolve([key, rates]);
        })
      );
  ...
  tasks.push(
    new Promise(async (resolve) => {
      ...
      resolve([
        ...
        await getRates(`data/price?fsym=${BTC}`),
      ]);
    })
  );
  ...
  console.log(
    `✅ Rebuilt CRYPTOS cache (${Object.keys(history).length} months) ...`
  );
